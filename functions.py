...
def import_results(path_prefix_, split_index_, cols_=['A1FREQ','BETA', 'SE'], \
cols__=['CHROM','GENPOS','ID','ALLELE0','ALLELE1']):
    if split_index_ == 1:
        df_temp = pd.read_csv(path_prefix_ % 1, usecols= cols__ + cols_, sep=" ", header=0)
        return (np.array(df_temp['BETA'], dtype=np.float32).reshape((-1,1,1)),\
np.array(df_temp['SE'], dtype=np.float32).reshape((-1,1,1)), df_temp[cols__+['A1FREQ']])
    else:
        df_temp = pd.read_csv(path_prefix_ % 1, usecols= cols__ + cols_, sep=" ", header=0)
...
def compute_cor_summary(beta_,se_,correction_bool=True):
    z_ = beta_ / se_
    alpha_ = np.array([[(z_[:,i,0] * z_[:,j,0])/np.abs(z_[:,i,0] * z_[:,j,0]) \
for i in range(len(z_[0]))] for j in range(len(z_[0]))]).transpose(2,0,1)
    if correction_bool:
        return alpha_ * np.repeat(np.array([[round(np.corrcoef(z_[:,i,0],z_[:,j,0])[0,1],13) \
for i in range(len(z_[0]))] for j in range(len(z_[0]))], dtype=np.float32)[np.newaxis, :, :], len(se_), axis=0)
    else:
        return np.repeat(np.array([[round(np.corrcoef(z_[:,i,0],z_[:,j,0])[0,1],13) \
for i in range(len(z_[0]))] for j in range(len(z_[0]))], dtype=np.float32)[np.newaxis, :, :], len(se_), axis=0)
