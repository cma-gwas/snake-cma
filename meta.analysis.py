...
parser.add_argument("--is_binary", help="Example Usage: --is_binary 1 (trait is a disease), or is_binary 0 (trait is continuous)", metavar="<0 or 1>")
...
#        X.to_csv(args.out + ".meta", sep=" ")
    ...
    if args.meta_summary:
        print('\nPerforming fixed-effect meta analysis with correction using summary statistics...\nSplit index is: %s' % args.split)
        if args.inflate:
            X = meta_summary_inflated(args.meta_summary, int(args.split), float(args.inflate), int(args.is_binary))
            X.to_csv(args.out + ".meta", sep=" ")
        else:
            X = meta_summary(args.meta_summary, int(args.split))
            X.to_csv(args.out + ".meta", sep=" ")
